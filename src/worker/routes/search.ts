...
import { contactRefColumns, likePattern, toContactRef } from "../services/contacts";
...
app.get("/search", zValidator("query", querySchema, validationHook), async (c) => {
  ...
  const byName = await db
    .select({ ...contactRefColumns, nickname: contacts.nickname })
    .from(contacts)
  ...
  if (hits.size < limit) {
    const byMethod = await db
      .select({ ...contactRefColumns, value: contactMethods.value })
      .from(contactMethods)
  ...
  if (hits.size < limit) {
    ...
    if (tagRows.length > 0) {
      const byTag = await db
        .select({ ...contactRefColumns, tagName: tags.name })
        .from(contactTags)
