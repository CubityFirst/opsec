...
The **Ask** tab is a chat over your CRM data ("When did I last speak to Alice, and what about?"). The model investigates with read-only tools (`search_contacts`, `get_contact`, `list_interactions`, `get_interaction`, `get_activity`, `list_life_events`, `list_bets`, `list_reminders`) and can *propose* any change (new interactions and contacts; edits to names, pronouns, birthday, how-we-met, job and employer, custom fields, tags, contact methods, relationships, life events, bets, reminders and existing interactions; settling bets; completing, skipping and reopening reminders; marking someone as deceased; archiving and deletions) that you apply with a button; it never writes on its own. When it needs a decision first ("Nobody called Sam Lee exists. Shall I create them?") it can offer a few one-tap replies under the question; tapping one sends it as your next message. Conversations live in the browser only. You can paste or attach a screenshot and ask the model to describe or log it.
